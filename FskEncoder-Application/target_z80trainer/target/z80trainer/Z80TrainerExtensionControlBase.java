...
public abstract class Z80TrainerExtensionControlBase  extends TargetSystemExtensionControl implements ChangeListener, FocusListener {

	private Logger logger = LogManager.getLogger(Z80TrainerExtensionControlBase.class.getName());

	protected final String DEFAULT_PROGRAM_NUMBER;
	
	protected Z80TrainerExtensionGui gui;	// NOSONAR
	
	protected Z80TrainerProtocolBase protocol;
	
	
	/**
	 * @param aWorkFlowEngine
	 */
	public Z80TrainerExtensionControlBase(StatusMessenger aStatusMessenger, final Z80TrainerGuiType aGuiType, final String aDefaultPrgNumber) {
		
		logger.trace("Z80TrainerExtensionControlBase(): StatusMessenger {}", aStatusMessenger);
		
		if(aStatusMessenger  == null) throw new IllegalArgumentException("aStatusMessenger can't be null");
		if(aDefaultPrgNumber == null) throw new IllegalArgumentException("aDefaultPrgNumber can't be null");
		
		DEFAULT_PROGRAM_NUMBER = aDefaultPrgNumber;
		
		gui = new Z80TrainerExtensionGui(this, aGuiType);
	...
	protected abstract void setProgramNumber();
